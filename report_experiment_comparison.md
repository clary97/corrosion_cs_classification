...
2. **부식 픽셀의 검출 누락 감소**: 4-class 모델에서는 부식이 존재하는 픽셀이라도 심각도 등급(Fair/Poor/Severe)을 잘못 예측하면 해당 클래스에서 미검출(False Negative)로 처리된다. 즉, 모델이 부식 자체는 올바르게 감지했음에도 불구하고 등급 분류 오류로 인해 Recall 손실이 발생한다. 이진 분류에서는 부식 픽셀을 어느 등급으로 예측하든 Corrosion으로 통합되므로, 이러한 누락이 사라져 Recall이 크게 향상된다(4-class 개별 클래스 Recall 0.56~0.69 → Binary Corrosion Recall 0.86).
3. **클래스 불균형 완화**: 세 클래스를 합치면 Corrosion 샘플(3,473,681 픽셀, 23.7%)이 충분히 확보되어 학습이 안정화된다. 4-class에서 Severe는 전체 픽셀의 2.8%에 불과했다.
4. **FPR 감소**: 부식 클래스가 단일화되어, 모델이 부식 픽셀을 Good으로 잘못 분류하는 오류가 감소하였다.
